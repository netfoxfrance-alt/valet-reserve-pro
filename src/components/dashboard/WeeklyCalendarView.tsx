...
export function WeeklyCalendarView({ currentDate, appointments, onAppointmentClick, onSlotClick, blockedPeriods = [] }: WeeklyCalendarViewProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-2xl border border-border overflow-hidden">
      {/* Single scrollable container — header is sticky inside */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        <table className="w-full border-collapse table-fixed" style={{ minWidth: 400 }}>
          <colgroup>
            <col style={{ width: TIME_COL }} />
          ...
          <thead className="sticky top-0 z-20 bg-card">
            <tr className="border-b border-border">
              <th style={{ width: TIME_COL }} />
              {weekDays.map((day, idx) => (
                <th
                  key={idx}
                  className={cn(
                    "text-center py-2.5 sm:py-3 border-l border-border font-normal",
          ...
          <tbody>
            {hours.map((hour, hourIdx) => (
              <tr key={hour}>
                {/* Time label */}
                <td
                  className="relative align-top select-none"
                  style={{ height: HOUR_HEIGHT }}
                >
                  <span className="absolute right-2 -top-[7px] text-[10px] sm:text-[11px] text-muted-foreground tabular-nums">
                    {`${hour}:00`}
                  </span>
                </td>
                ...
                {weekDays.map((day, dayIdx) => {
                  ...
                  // Only render appointments in first hour row to avoid duplicates
                  ...
                  return (
                    <td
                      key={dayIdx}
                      className={cn(
                        "relative border-l border-t border-border/50 p-0",
                    ...
                    >
                      ...
                      {/* Appointments — rendered only from the first row, positioned absolutely over full column */}
                      {dayApts.map(apt => {

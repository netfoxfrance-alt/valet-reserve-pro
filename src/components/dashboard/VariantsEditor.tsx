...
import { Plus, Trash2, ImageIcon, X, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
interface VariantsEditorProps {
  ...
  onAdd: () => void;
  onUpdate: (index: number, field: 'name' | 'price' | 'image_url' | 'description', value: string | number | null) => void;
      ...
      {variants.length === 0 ? (
        ...
        <div className="space-y-3">
          {variants.map((variant, index) => (
            <div key={index} className="rounded-lg border border-border/50 bg-secondary/10 overflow-hidden">
              ...
              {expandedIdx === index && (
                <div className="px-3 pb-3 pt-0">
                  ...
                  <RichTextEditor
                    ...
                    onChange={(html) => onUpdate(index, 'description', html || null)}
                    placeholder="Décrivez cette variante..."
                    className="min-h-0 [&_.ProseMirror]:min-h-[80px]"
                  />
                </div>
              )}
            </div>

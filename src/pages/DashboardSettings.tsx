  ...
  return (
    <DashboardLayout title={t('settings.title')}>
        <div className="max-w-2xl">
          ...
          {/* Payments Mode Toggle (Test / Live) */}
          <section className="mb-6 sm:mb-8">
            <h2 className="text-lg sm:text-xl font-semibold text-foreground mb-1 sm:mb-2">{t('settings.paymentsMode', 'Mode de paiement')}</h2>
            <p className="text-sm text-muted-foreground mb-4 sm:mb-6">{t('settings.paymentsModeDesc', 'Basculez en mode Test pour tester les paiements avec des cartes de test Stripe sans impact sur le live.')}</p>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    center?.payments_mode === 'test' ? "bg-amber-500/10" : "bg-emerald-500/10"
                  )}>
                    <CreditCard className={cn(
                      "w-5 h-5",
                      center?.payments_mode === 'test' ? "text-amber-500" : "text-emerald-500"
                    )} />
                  </div>
                  <div>
                    <p className="font-medium text-foreground">
                      {center?.payments_mode === 'test' 
                        ? t('settings.testMode', '🧪 Mode Test actif')
                        : t('settings.liveMode', '🟢 Mode Live actif')}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {center?.payments_mode === 'test'
                        ? t('settings.testModeDesc', 'Les paiements utilisent les clés de test Stripe. Aucun vrai débit.')
                        : t('settings.liveModeDesc', 'Les paiements sont réels. Les clients sont débités.')}
                    </p>
                  </div>
                </div>
                <Select
                  value={center?.payments_mode || 'live'}
                  onValueChange={async (value) => {
                    const { error } = await updateCenter({ payments_mode: value as 'live' | 'test' });
                    if (error) {
                      toast({ title: t('common.error'), description: error, variant: 'destructive' });
                    } else {
                      toast({ 
                        title: value === 'test' 
                          ? t('settings.switchedToTest', 'Mode Test activé')
                          : t('settings.switchedToLive', 'Mode Live activé'),
                        description: value === 'test'
                          ? t('settings.switchedToTestDesc', 'Vous pouvez maintenant tester les paiements sans impact.')
                          : t('settings.switchedToLiveDesc', 'Les paiements sont maintenant réels.'),
                      });
                      // Reload to refresh Connect status for the new mode
                      window.location.reload();
                    }
                  }}
                >
              ...
              {center?.payments_mode === 'test' && (
                <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                  <p className="text-sm text-amber-700 dark:text-amber-400">
                    ⚠️ {t('settings.testWarning', 'En mode test, vous devrez créer un nouveau compte Connect Express (environnement test). Les comptes test et live sont séparés chez Stripe. Utilisez la carte 4242 4242 4242 4242 pour tester.')}
                  </p>
                </div>
              )}
            </Card>
